...
const ActionGroupPage = () => {
  ...
  const validateConfiguration = () => {
    ...
    if (apiConfig.endpoints.length === 0) errors.push('At least one endpoint is required');
  ...
  const handleCreateActionGroup = async () => {
    ...
    try {
      const response = await actionGroupAPI.createActionGroup(apiConfig);
      ...
      setCreationResult({
        success: false,
        message: error.message || 'An error occurred while creating the action group'
      });
    } finally {
      setIsCreating(false);
    }
  ...
  const handleTestActionGroup = async (actionGroupId) => {
    try {
      ...
      alert(response.success ? 'Test successful!' : 'Test failed: ' + response.error);
    } catch (error) {
  ...
  const handleDeleteActionGroup = async (actionGroupId) => {
    const actionGroup = existingActionGroups.find(ag => ag.actionGroupId === actionGroupId);
    ...
    if (confirm(confirmMessage)) {
      ...
      try {
        ...
        if (response.success) {
          await loadExistingActionGroups();
          alert('Action group deleted successfully' + (isEnabled ? ' (automatically disabled first)' : ''));
        } else {
  ...
  const handleEditActionGroup = async (actionGroupId) => {
    try {
      ...
      if (response.success) {
        ...
        alert('Edit functionality will be available in the next update. For now, you can create a new action group with updated configuration.');
      } else {
  ...
  return (
    <div className="container mx-auto px-4 py-6">
      <div className="mb-6">
        <h1 className="text-3xl font-bold text-gray-900">Action Group Management</h1>
            ...
            <div className="grid grid-cols-2 gap-4 mt-2 text-sm">
              ...
              <div>
                ...
                <span className={`ml-2 px-2 py-1 rounded text-xs ${
                  agentInfo.status === 'PREPARED' ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800'
                }`}>
                  {agentInfo.status}
                </span>
              </div>
      ...
      {activeTab === 'create' && (
        <div className="space-y-6">
          ...
          <div className="card bg-gradient-to-r from-blue-50 to-indigo-50 border-blue-200">
            <div className="p-6">
              <h2 className="text-xl font-semibold mb-4 text-blue-800">📋 Step-by-Step Guide</h2>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <div className="font-medium text-blue-800">Configure API</div>
                    <div className="text-gray-600">Enter API name, base URL, and description</div>
                  </div>
                </div>
                <div className="flex items-start gap-3">
                  <div className="w-6 h-6 bg-blue-600 text-white rounded-full flex items-center justify-center font-bold text-xs">2</div>
                  <div>
                    <div className="font-medium text-blue-800">Add Endpoints</div>
                    <div className="text-gray-600">Create endpoints, add parameters, then click "Add Endpoint"</div>
                  </div>
                </div>
                <div className="flex items-start gap-3">
                  <div className="w-6 h-6 bg-blue-600 text-white rounded-full flex items-center justify-center font-bold text-xs">3</div>
                  <div>
                    <div className="font-medium text-blue-800">Create Action Group</div>
                    <div className="text-gray-600">Review your configuration and create the action group</div>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className="card">
            <div className="p-6">
              <h2 className="text-xl font-semibold mb-4">Step 1: API Configuration</h2>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setApiConfig({...apiConfig, apiName: e.target.value})}
                  ...
                  <div>
                    ...
                    <select
                      ...
                      onChange={(e) => setApiConfig({
                        ...apiConfig,
                        authentication: {...apiConfig.authentication, type: e.target.value}
                      })}
                    >
                    ...
                    <>
                      <div>
                        ...
                        <select
                          ...
                          onChange={(e) => setApiConfig({
                            ...apiConfig,
                            authentication: {...apiConfig.authentication, location: e.target.value}
                          })}
                        >
                      ...
                      <div>
                        ...
                        <input
                          ...
                          onChange={(e) => setApiConfig({
                            ...apiConfig,
                            authentication: {...apiConfig.authentication, name: e.target.value}
                          })}
                      ...
                      <div>
                        ...
                        <input
                          ...
                          onChange={(e) => setApiConfig({
                            ...apiConfig,
                            authentication: {...apiConfig.authentication, value: e.target.value}
                          })}
          ...
          <div className="card">
            <div className="p-6">
              <h2 className="text-xl font-semibold mb-4">Step 2: Endpoints Configuration</h2>
              ...
              <div className="border border-gray-200 rounded-lg p-4 mb-4 bg-gray-50">
                ...
                <div className="mb-4">
                  ...
                  <div className="border-2 border-dashed border-blue-200 rounded-lg p-4 bg-blue-50">
                    <div className="flex items-center justify-between mb-3">
                      <h5 className="font-medium text-blue-800">Add Parameter</h5>
                      {currentEndpoint.path.includes('{') && currentEndpoint.path.includes('}') && (
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-6 gap-2 mb-3">
                      ...
                      <select
                        ...
                        onChange={(e) => setCurrentParameter({...currentParameter, type: e.target.value})}
                      >
                      ...
                      <select
                        ...
                        onChange={(e) => setCurrentParameter({...currentParameter, location: e.target.value})}
                      >
                    ...
                    {currentParameter.name.trim() && (
                      <div className="text-sm text-blue-600 mb-2">
                        💡 Click "Add Parameter" to save this parameter, then add more if needed
                      </div>
                    )}
                  </div>
                  ...
                  {currentEndpoint.parameters.length > 0 ? (
                    <div className="mt-4">
                      <div className="flex items-center gap-2 mb-2">
                        <h5 className="font-medium text-green-700">✅ Added Parameters ({currentEndpoint.parameters.length})</h5>
                      </div>
                      <div className="space-y-2">
                        {currentEndpoint.parameters.map((param, index) => (
                          <div key={index} className="flex items-center justify-between bg-green-50 border border-green-200 p-3 rounded">
                            <span className="text-sm">
                              ...
                              {param.description && <span className="text-gray-600 ml-2">- {param.description}</span>}
                            </span>
                            <button
                    ...
                    <div className="mt-4 p-3 bg-yellow-50 border border-yellow-200 rounded">
                      <div className="text-sm text-yellow-700">
                        ...
                        {currentEndpoint.path.includes('{') && currentEndpoint.path.includes('}') && (
                          <span className="font-medium"> Your URL path contains parameters - make sure to add them above!</span>
                        )}
                      </div>
                    </div>
                  )}
                ...
                <div className="mb-4">
                  ...
                  <textarea
                    ...
                    onChange={(e) => setCurrentEndpoint({...currentEndpoint, responseExample: e.target.value})}
                ...
                <div className="flex flex-col gap-2">
                  ...
                  {currentEndpoint.path.includes('{') && 
                   currentEndpoint.path.includes('}') && 
                   !currentEndpoint.parameters.some(p => p.location === 'path') && (
                    <div className="p-3 bg-red-50 border border-red-200 rounded">
                      <div className="text-sm text-red-700">
                        ⚠️ <strong>Warning:</strong> Your URL path contains parameters (e.g., {currentEndpoint.path.match(/\{[^}]+\}/g)?.join(', ')}) 
                        but no path parameters are defined. Make sure to add them above before creating the endpoint.
                      </div>
                    </div>
                  )}
                  ...
                  <button
                    type="button"
                    onClick={addEndpoint}
                    disabled={!currentEndpoint.path.trim() || !currentEndpoint.description.trim()}
                    className={`px-6 py-3 rounded font-medium transition-colors ${
                      currentEndpoint.path.trim() && currentEndpoint.description.trim()
                  ...
                  >
                    ...
                    {currentEndpoint.parameters.length > 0 && (
                      <span className="ml-2 px-2 py-1 bg-green-500 text-green-100 rounded text-sm">
                        {currentEndpoint.parameters.length} param{currentEndpoint.parameters.length !== 1 ? 's' : ''}
                      </span>
                    )}
                  </button>
                </div>
              ...
              {apiConfig.endpoints.length > 0 && (
                <div className="space-y-3">
                  <h3 className="font-medium text-green-700">✅ Configured Endpoints ({apiConfig.endpoints.length})</h3>
          ...
          <div className="card">
            <div className="p-6">
              <h2 className="text-xl font-semibold mb-4">Step 3: Create Action Group</h2>
              {/* Progress Summary */}
              <div className="mb-4 p-4 bg-blue-50 border border-blue-200 rounded-lg">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
                  <div>
                    ...
                    <div className={apiConfig.apiName.trim() ? 'text-green-600' : 'text-red-500'}>
                      {apiConfig.apiName.trim() ? `✅ ${apiConfig.apiName}` : '❌ Not set'}
                    </div>
                  </div>
                  <div>
                    <span className="font-medium">Base URL:</span>
                    <div className={apiConfig.baseUrl.trim() ? 'text-green-600' : 'text-red-500'}>
                      {apiConfig.baseUrl.trim() ? `✅ ${apiConfig.baseUrl}` : '❌ Not set'}
                    </div>
                  </div>
                  <div>
                    <span className="font-medium">Endpoints:</span>
                    <div className={apiConfig.endpoints.length > 0 ? 'text-green-600' : 'text-red-500'}>
                      {apiConfig.endpoints.length > 0 ? `✅ ${apiConfig.endpoints.length} endpoint${apiConfig.endpoints.length !== 1 ? 's' : ''}` : '❌ No endpoints'}
                    </div>
                  </div>
                </div>
                ...
                {apiConfig.endpoints.length > 0 && (
                  <div className="mt-3 pt-3 border-t border-blue-200">
                    <span className="font-medium text-sm">Parameters Summary:</span>
                    <div className="text-sm text-gray-600 mt-1">
                      {apiConfig.endpoints.map((endpoint, index) => (
                        <div key={index} className="flex items-center gap-2">
                          <span className="font-medium">{endpoint.method} {endpoint.path}:</span>
                          <span className={endpoint.parameters.length > 0 ? 'text-green-600' : 'text-yellow-600'}>
                            {endpoint.parameters.length > 0 
              ...
              <div className="flex items-center justify-between">
                <div>
                  <h3 className="font-semibold">Ready to Create Action Group?</h3>
                  <p className="text-sm text-gray-600">
                    This will automatically generate OpenAPI schema and create the action group in AWS Bedrock.
                  </p>
                </div>
                <button
                  onClick={handleCreateActionGroup}
                  disabled={isCreating || !agentInfo || apiConfig.endpoints.length === 0}
                  className={`px-6 py-3 rounded font-medium transition-colors ${
                    !isCreating && agentInfo && apiConfig.endpoints.length > 0
                      ? 'bg-blue-600 text-white hover:bg-blue-700'
                      : 'bg-gray-300 text-gray-500 cursor-not-allowed'
                  }`}
                >
                ...
                <div className={`mt-4 p-4 rounded-lg ${
                  ...
                  {creationResult.success && creationResult.data && (
                    <div className="mt-2 text-sm">
                      <p>Action Group ID: {creationResult.data.actionGroupId}</p>
      ...
      {activeTab === 'manage' && (
        <div className="card">
          <div className="p-6">
            <h2 className="text-xl font-semibold mb-4">Existing Action Groups</h2>
            
            {existingActionGroups.length === 0 ? (
              <div className="text-center py-8 text-gray-500">
                <p>No action groups found. Create your first one using the form above.</p>
              </div>
            ) : (
              <div className="space-y-4">
                {existingActionGroups.map((actionGroup) => (
                  <div key={actionGroup.actionGroupId} className="border border-gray-200 rounded-lg p-4">
                    <div className="flex items-center justify-between mb-2">
                      <div>
                        <h3 className="font-semibold">{actionGroup.actionGroupName}</h3>
                        <p className="text-sm text-gray-600">{actionGroup.description}</p>
                      </div>
                      <div className="flex space-x-2">
                        <button
                          onClick={() => handleToggleActionGroup(actionGroup.actionGroupId, actionGroup.actionGroupState)}
                        ...
                        >
                          {actionGroup.actionGroupState === 'ENABLED' ? 'Disable' : 'Enable'}
                        </button>
                        <button
                          onClick={() => handleTestActionGroup(actionGroup.actionGroupId)}
                          className="btn-secondary text-sm"
                        >
                          Test
                        </button>
                        <button
                          onClick={() => handleEditActionGroup(actionGroup.actionGroupId)}
                          className="bg-blue-500 text-white px-3 py-1 rounded text-sm hover:bg-blue-600"
                        >
                          Edit
                        </button>
                        <button
                          onClick={() => handleDeleteActionGroup(actionGroup.actionGroupId)}
                        ...
                        >
                          {isDeleting === actionGroup.actionGroupId ? 'Deleting...' : 'Delete'}
                        </button>
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-3 gap-4 text-sm">
                      ...
                      <div>
                        <span className="font-medium">Created:</span> {new Date(actionGroup.createdAt).toLocaleDateString()}
                      </div>
                      <div>
                        <span className="font-medium">Updated:</span> {new Date(actionGroup.updatedAt).toLocaleDateString()}
                      </div>
                    </div>
                  </div>
                ))}
